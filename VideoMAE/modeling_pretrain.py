...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward(self, rgb_x, depth_x, rgb_mask, depth_mask):
        ### added rgb and depth channels
        x = self.forward_features(rgb_x, depth_x, rgb_mask, depth_mask)
        x = self.head(x)
        return x
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        super().__init__()
        self.num_classes = num_classes
        assert num_classes == 4 * tubelet_size * patch_size ** 2  ### changed 3 channels to 4 channels
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
